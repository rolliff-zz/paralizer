...
namespace Paralizer
{
    ...
    "fort"};
        ...
        public static Unit FromProperty(JProperty prop)
        {
            ...
            foreach(var t in terra )
            {
                ...
                JToken tok;
                UnitTerrainModifier terrain = new UnitTerrainModifier()
                { type=t, attack=0,movement=0,defence=0};
    ...
    class MySerializer : JsonConverter
    {
    ...
    {
        public static List<Unit> ProcessUnits(JObject unit)
        {
            List<Unit> list = new List<Unit>();
            foreach (var o in unit.Properties())
            {
                list.Add(Unit.FromProperty(o));
            }
            return list;
        }
    }
